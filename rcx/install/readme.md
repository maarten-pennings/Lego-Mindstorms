...
This document describes installing an IDE for the RCX (LEGO Mindstorms Robotics Invention System)
...
Both require a bit of hardware to hookup the infra red tower.
...
select a slot with the _Pgrm_ key and run and stop that with the _Run_ key. 
...
Also, in boot mode
 - The _View_ button is not working.
   I think the view application is a sixth application, which comes with
   the flex firmware. I assume the view apllication is part of the flex firmware 
   and the not fixed firmware, is that LEGO can now add other sensors/actuators
   to the eco system, and update the view application to support viewing them.
   
   The _View_ button needs the flex firmware, and the sensor ports needs to be 
...
They come with CDs which contain that firmware.

I also have an USB-to-IDE-adapter.
It is normally used to connect a bare harddisk to a PC via USB.
But I once rescued a DVD reader from an old desktop.
...
to your laptop. My IR towers are from mindstorms robot invention 1.0 and 1.5. 
...
The mindstrorms robot invention sets come with a female-to-female serial cable.
...
MAX232 level shifter, anyhow it works for me.

![Device manager](images/device-manager.png)

### IDE

With the hardware problem out of the way, we are ready for the software.
...
The RCX is now likely in _boot mode_ (see Introduction, RCX display 
does not show time left of the man): we just placed batteries, so it contains no
...
after wiating 10 seconds we can switch it to `Short` (and see the green LED on the tower.
...
between tower and RCX does not match the distance settings (switch on Tower, configured, 
see above in RCX) or when there is too much ambient IR light (switch off lights, close
the curtains).
...
save this somewhere with extension `nqc` (!). Only then Compile > Compile.
...
Note that this will upload the compiled program to the slot shown on the ribbon.

![Download application](images/download-application.png)

Finally, on the RCX, select slot 1 with the _Prgm_ button and the 
